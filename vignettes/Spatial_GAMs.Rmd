...
spatial_dat <-
   read_excel(
      system.file("extdata", "SpatioTemporalSpreadData.xlsx", package = "spatiotemporaldynamics"),
      sheet = 1
   )
```
...
The variable `total_rain` should have shown a linear trend.
Upon closer inspection of data, rainfall amount increased gradually with time at Billa Billa.
However, this wasn't the case at Tosari, where more rain occurred at the beginning, less frequent at the middle season and then one final big rain occurred towards the end of the season.
It should a bell shape trend for Tosari, not downward trend though?  
...
By looking at the plots of both models, the relationship seem very complex in `model_4`.
The `model 4.1` shows that `avg_wind_speed` has more influence at `Billa Billa`.
We had only two assessments for spatial spread - more assessments are required to accurately determine data patterns.
...
model_8 <-
  gam(
    incidence ~ s(total_rain, k = 3) +
      s(distance, k = 3) +
      s(avg_wind_speed, k = 3) +
      s(avg_rh, k = 3) +
      s(avg_temp, k = 3) +
      location + assessment_number + direction,
    data = spatial_dat,
    method = "REML"
  )
...
model_9 <-
  gam(
    incidence ~ s(total_rain, k = 3, by = location) +
      s(distance, k = 3, by = location) +
      s(avg_wind_speed, k = 3, by = location)
    + s(avg_rh, k = 3, by = location) +
      s(avg_temp, k = 3, by = location) +
      assessment_number +
      direction,
...
By fitting different smooths for each term by location, `total_rain` , `avg_wind_speed` and `avg_rh` are no longer significant in model_9, but adjusted R^2 and Deviance Explained values improved.
...
### Model 12 (Interaction between total_rain and avg_wind_speed by location)

```{r fit model-12}
model_12 <-
  gam(
    incidence ~ s(total_rain, avg_wind_speed, k = 19, by = location) +
      s(distance, k = 3, by = location) +
      s(avg_temp, k = 3, by = location) +
      assessment_number +
      direction,
...
### Visualize the interaction between avg_wind_speed and total_rain of model_12
...
### Model 13 (Interaction between total_rain and avg_wind_speed NOT by location)

```{r fit model-13}
model_13 <-
  gam(
    incidence ~ s(total_rain, avg_wind_speed, k = 19) +
      s(distance, k = 3) +
      s(avg_rh, k = 3) +
      location +
      assessment_number +
...
### Model 14 (Try Tensor interaction between total_rain and avg_wind_speed NOT by location)

```{r model-14}
model_14 <-
  gam(
    incidence ~ ti(total_rain, avg_wind_speed, k = 16) +
      s(distance, k = 3) +
      s(avg_rh, k = 3) +
      location +
      assessment_number +
...
<!-- This is not true when using optimal `k` values for the models... -->
Both `model_12` and `model_10` have same AIC, BIC and adjusted R^2 values.
Both rain and wind speed are not significant in `model_10`, but these are significant in `model_12`.
Therefore `model_12` should be chosen because it makes more sense and will be easier to interpret in Results/Discussions. 
