...
model_14 <-
  gam(
    incidence ~ s(total_rain, avg_wind_speed, k = 18, by = location) +
      s(distance, k = 4, by = location) +
      s(avg_temp, k = 18, by = location) +
      assessment_number + location + direction,
    data = dat,
    method = "REML"
  )
...
# Generate a new plot.gam object just for the publication 
```{r}
p16 <- p_gam(x = getViz(model_16))
```


# Save png and eps files of the best fit model plot

```{r visualise-best-fit-model}
png(
   file = here::here("man/figures", "Fig2.png"),
   width = 640,
   height = 640,
   units = "px",
   pointsize = 14
...
postscript(file = here::here("man/figures", "Fig2.eps"),
           family = "Arial")
...
embed_fonts(
   file = here::here("man/figures", "Fig2.eps"),
   outfile = here::here("man/figures", "Fig2.eps"),
   options = "-dEPSCrop"
)
