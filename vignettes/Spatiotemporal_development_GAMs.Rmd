...
Wind direction for the Billa Billa site is the character format. Convert it to degress for calculations

```{r convert-text-to-degrees}
dt %>%
  mutate(
    wind_direc = case_when(
  ...
  mutate(wind_direc = as.numeric(wind_direc)) %>%
  group_by(assessment_number, location) %>%
  summarise(circular.averaging(wind_direc)) %>%
  rename(wind_direc = 'circular.averaging(wind_direc)')

```


## Copy `wind_direc` column to the spatiotemporal spread data

```{r}
dat <- dat %>%
  mutate(
    wind_direc = case_when(
      location == "Billa Billa" & assessment_number == 1 ~ 88.5,
      location == "Billa Billa" &
        assessment_number == 2 ~  110,
      location == "Billa Billa" &
        assessment_number == 3 ~  239,
      location == "Billa Billa" &
        assessment_number == 4 ~  84.5,
      location == "Billa Billa" &
        assessment_number == 5 ~  207,
      location == "Billa Billa" &
        assessment_number == 6 ~  45.9,
      location == "Billa Billa" &
        assessment_number == 7 ~  63.5,
      location == "Billa Billa" &
        assessment_number == 8 ~  56.5,
      location == "Billa Billa" &
        assessment_number == 9 ~  77.3,
      location == "Billa Billa" &
        assessment_number == 10 ~  359,
      location == "Tosari" &
        assessment_number == 1 ~  195,
      location == "Tosari" &
        assessment_number == 2 ~  176,
      location == "Tosari" &
        assessment_number == 3 ~  241,
      location == "Tosari" &
        assessment_number == 4 ~  131,
      location == "Tosari" &
        assessment_number == 5 ~  137,
      location == "Tosari" &
        assessment_number == 6 ~  125,
      location == "Tosari" &
        assessment_number == 7 ~  119,
      location == "Tosari" &
        assessment_number == 8 ~  165
    )
  ) %>%
  mutate(wind_direc = as.numeric(wind_direc))
...
### Combine direction & location columns

This provides information on whether the rate of disease progress was significantly faster in quadrats located in a particular direction.

```{r combine-location-and-direction-column}
dat<-
...
Tensor interaction takes longer to run the model, and requires more data. 
...
The `model_16` seems to be the best fit model based on lower AIC/BIC values and high adjusted R^2^ value.
